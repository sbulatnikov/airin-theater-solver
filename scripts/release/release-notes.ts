...
import { type GitHubPullRequest, pullRequest } from "../shared/github.ts";
...
export async function generateReleaseNotes(
  ...
  const output = run("git", ["log", "--reverse", `--format=${releaseLogFormat}`, `${previousTag}..${target}`]);
  ...
  const changes = await Promise.all(
    commits.map(async (commit): Promise<ReleaseChange> => {
      ...
      return { commit, pullRequest: await pullRequest(repository, number, token) };
    })
  );
  return formatReleaseNotes(changes, repository);
}
