...
`Release notes`, `Build` и `E2E tests`. История линейная, force push и удаление запрещены. GitHub Actions App является
единственным bypass-актором и использует это право только для автоматически сгенерированного release-коммита.
