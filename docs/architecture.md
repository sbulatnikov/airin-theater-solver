...
Связей `apps/v1 → core/v2` и `apps/v2 → core/v1` нет. Скрипт `scripts/assemble-dist.mjs` дополнительно проверяет, что итоговый HTML не содержит номер чужой версии.
